...
| ------------- | ------------- | ------------- | ------------- |
| **GET** | /videos | List videos  | `scope` [public, private, unlisted], `status` [pending, ok, processing, failed], `state` [active, inactive, frozen] `limit`, `start` [offset to add], `sort_by` [fild to sort by], `order_by` [DESC, ASC]
